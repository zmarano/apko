...
// GroupFile describes an entire /etc/shadow file's contents.
...
func (se *ShadowEntry) Parse(line string) error {
	...
	se.UserName = parts[0]
	se.EncPassword = parts[1]
	se.LastChanged = parts[2]
	se.MinPassAge = parts[3]
	se.MaxPassAge = parts[4]
	se.WarnPeriod = parts[5]
	se.InactivityPeriod = parts[6]
	se.AcctExpiry = parts[7]
	se.Unused = parts[8]
...
func (se *ShadowEntry) Write(w io.Writer) error {
	_, err := fmt.Fprintf(w, "%s:%s:%s:%s:%s:%s:%s:%s:\n", se.UserName, se.EncPassword, se.LastChanged, se.MinPassAge, se.MaxPassAge, se.WarnPeriod, se.InactivityPeriod, se.AcctExpiry)
	return err
}
